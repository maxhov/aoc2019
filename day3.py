...
def points_in_segment(point, translation):
    direction = translation[0]
    length = int(translation[1:])

    if direction == 'R':
        return [[x, point[1]] for x in range(point[0] + 1, point[0] + length + 1)]

    if direction == 'L':
        return [[x, point[1]]
                for x in range(point[0]-1, point[0]-length-1, -1)]

    if direction == 'U':
        return [[point[0], x] for x in range(point[1]+1, point[1]+length+1)]

    if direction == 'D':
        return [[point[0], x]
                for x in range(point[1]-1, point[1]-length-1, -1)]


def compute_path(starting_point, translation):
    path = [starting_point]
    for translation in translation:
        path.extend(points_in_segment(path[-1], translation))

    return path


def intersects(list1, list2):
    return [val for val in list1 if val in list2 if val != [0, 0]]
